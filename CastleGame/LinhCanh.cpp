...
LinhCanh::LinhCanh(DXTexture *pTexture, D3DXVECTOR2 position, Direction direction, D3DXVECTOR2 velocity)
	: Enemy(pTexture, position, direction, velocity)
{
	...
	this->_OType = EObjectName::LINHCANH_OBJECT;
...
void LinhCanh::Update(GameTime *gameTime)
{
	DynamicObject::Update(gameTime);
}

void LinhCanh::Draw(DXGame *pDXGame, Camera *pCamera)
{
	LivingObject::Draw(pDXGame, pCamera);
}

void LinhCanh::UpdateObjectState(GameTime *gameTime)
{
