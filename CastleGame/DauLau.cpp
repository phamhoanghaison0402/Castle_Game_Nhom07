...
DauLau::DauLau(DXTexture *pTexture, D3DXVECTOR2 position, Direction direction, D3DXVECTOR2 velocity)
	: Enemy(pTexture, position, direction, velocity)
{
	...
	this->_OType = EObjectName::DAULAU_OBJECT;
...
void DauLau::Update(GameTime *gameTime)
{
	DynamicObject::Update(gameTime);
}

void DauLau::Draw(DXGame *pDXGame, Camera *pCamera)
{
	LivingObject::Draw(pDXGame, pCamera);
}

void DauLau::UpdateObjectState(GameTime *gameTime)
{
